...
impl Parseable<MainChunk> for MainChunk {
  fn parse<R: Read>(rdr: &mut R) -> Result<MainChunk, ParserError> {
    let mut chunk: MainChunk = MainChunk { map_area_info: [SMAreaInfo::default(); 4096] };
...
impl From<&IffChunk> for MWMOChunk {
  fn from(value: &IffChunk) -> Self {
    ...
    if size == 0 {
      return MWMOChunk { filename: String::new() };
    }

    MWMOChunk {
      filename: CStr::from_bytes_with_nul(&value.data)
        .map_err(|_| ParserError::FormatError { reason: "Cannot convert MWMOChunk to valid UTF-8" })
