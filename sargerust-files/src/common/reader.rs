...
impl Parseable<u8> for u8 {
  fn parse<R: Read>(rdr: &mut R) -> Result<u8, ParserError> { Ok(rdr.read_u8()?) }
}

impl Parseable<i8> for i8 {
  fn parse<R: Read>(rdr: &mut R) -> Result<i8, ParserError> { Ok(rdr.read_i8()?) }
}
...
impl Parseable<i16> for i16 {
  fn parse<R: Read>(rdr: &mut R) -> Result<i16, ParserError> { Ok(rdr.read_i16::<LittleEndian>()?) }
}
...
impl Parseable<u128> for u128 {
  fn parse<R: Read>(rdr: &mut R) -> Result<u128, ParserError> { Ok(rdr.read_u128::<LittleEndian>()?) }
}
...
pub(crate) fn get_mandatory_chunk_by_name<T: Parseable<T>>(chunk_list: &Vec<IffChunk>, chunk_magic: &str) -> Result<T, ParserError> {
  chunk_list.iter()
...
pub(crate) fn get_optional_chunk_by_name<T: Parseable<T>>(chunk_list: &Vec<IffChunk>, chunk_magic: &str) -> Result<Option<T>, ParserError> {
  chunk_list.iter()
    .find(|chunk| chunk.magic_str().eq(chunk_magic))
