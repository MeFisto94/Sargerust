...
use crate::m2::types::{FOURCC_M2HEADER, FOURCC_M2SKIN, M2Array, M2Asset, M2SkinProfile, M2Texture, M2TextureInternal, M2Vertex, Version};
...
impl M2Reader {
  ...
  pub fn parse_asset<R: Read + Seek>(rdr: &mut R) -> Result<M2Asset, ParserError> {
    ...
    if version.major != 1 {
      return Err(ParserError::FormatError { reason: "M2Version.major MUST BE 1"})
    }

    #[cfg(feature = "wotlk")]
    if version.minor != 8 {
      return Err(ParserError::FormatError { reason: "M2Version.minor MUST BE 8 for WotLK" })
    }
    ...
    let textures: Vec<M2Texture> = texs.iter().map(|tex| M2Texture {
