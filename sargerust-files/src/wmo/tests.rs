  ...
  fn parse_root() -> Result<(), anyhow::Error> {
    ...
    let mut file = BufReader::new(File::open(test_data.join("World_wmo_Dungeon_AZ_Subway_Subway.wmo"))?);
  ...
  fn parse_group() -> Result<(), anyhow::Error> {
    let test_data = std::env::current_dir()?.join("test-data");
    let mut file = BufReader::new(File::open(test_data.join("World_wmo_Dungeon_AZ_Subway_Subway_000.wmo"))?);
