...
use crate::wmo::types::{MFOGChunk, MOBAChunk, MOBNChunk, MOBRChunk, MOCVChunk, MODDChunk, MODNChunk, MODRChunk, MODSChunk, MOGIChunk, MOGNChunk, MOGPChunk, MOHDChunk, MOLRChunk, MOLTChunk, MOMTChunk, MONRChunk, MOPYChunk, MOSBChunk, MOTVChunk, MOTXChunk, MOVIChunk, MOVTChunk, WMOGroupAsset, WMORootAsset};
...
impl WMOReader {
  pub fn parse_root<R: Read>(rdr: &mut R) -> Result<WMORootAsset, ParserError> {
    ...
    let momt_chunk = chunk_list.iter()
      .find(|chunk| chunk.magic_str().eq("MOMT"))
      .expect("Missing mandatory MOMT chunk");
    if momt_chunk.size % 64 != 0 {
      return Err(ParserError::FormatError { reason: "Invalid MOMT Chunk Size" });
    }
  ...
  fn get_mandatory_chunk_by_name<T: Parseable<T>>(chunk_list: &Vec<IffChunk>, chunk_magic: &str) -> Result<T, ParserError> {
    chunk_list.iter()
  ...
  fn get_optional_chunk_by_name<T: Parseable<T>>(chunk_list: &Vec<IffChunk>, chunk_magic: &str) -> Result<Option<T>, ParserError> {
    chunk_list.iter()
      .find(|chunk| chunk.magic_str().eq(chunk_magic))
