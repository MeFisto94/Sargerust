...
pub struct MapManager {
    ...
    pub loaded_tiles: HashMap<(u8, u8), (Arc<ADTAsset>, /* Terrain */ Vec<(Vec3, Mesh)>, Vec<PlacedDoodad>, /* WMO */ Vec<(Affine3A, Vec<(MeshWithLod, Vec<Material>)>)>, HashMap<String, BlpImage>)>
}
...
impl MapManager {
    ...
    pub fn preload_map(&mut self, map: String, position: Vec3, orientation: f32) {
        ...
        if wdt.has_chunk(chunk_coords.1, chunk_coords.0) {
            ...
            self.loaded_tiles.insert(chunk_coords, (adt.into(), terrain_chunk, render_list, wmos, texture_map));
