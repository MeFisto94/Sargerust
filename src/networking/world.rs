...
impl WorldServer {
    pub fn new(stream: TcpStream, encrypter: ClientEncrypterHalf, decrypter: ClientDecrypterHalf, packet_handler_sender: Sender<Box<ServerOpcodeMessage>>) -> Self {
        Self {
            stream,
    ...
    pub fn run(&self) {
        // TODO: this does not support enabled warden, as warden is installed before the auth response apparently
        {
            ...
            assert_eq!(0x02EFu16, skip_encrypted(self.stream(), dec.deref_mut()).unwrap());
            expect_server_message_encryption::<SMSG_CLIENTCACHE_VERSION, _>(&mut self.stream(), dec.deref_mut())
                .unwrap();
            expect_server_message_encryption::<SMSG_TUTORIAL_FLAGS, _>(&mut self.stream(), dec.deref_mut())
                .unwrap();

            CMSG_CHAR_ENUM {}.write_encrypted_client(self.stream(), enc.deref_mut()).unwrap();

            let s = expect_server_message_encryption::<SMSG_CHAR_ENUM, _>(&mut self.stream(), dec.deref_mut())
                .unwrap();
            info!("Characters: {:?}", &s.characters.iter().map(|char| &char.name).collect_vec());

            if s.characters.is_empty() {
                panic!("This account doesn't have any characters yet, please create exactly one");
            }

            CMSG_PLAYER_LOGIN { guid: s.characters[0].guid, }
                .write_encrypted_client(self.stream(), enc.deref_mut()).unwrap();
        }

        loop {
            let opcode_opt = ServerOpcodeMessage::read_encrypted(self.stream(), self.decrypter.lock().unwrap().deref_mut()).map(Box::new);
            match opcode_opt {
                Err(err) =>
                    match err {
                        ExpectedOpcodeError::Opcode{.. } => warn!("Unimplemented opcode: {}", err),
                        ExpectedOpcodeError::Parse(error)  => warn!("Parsing error: {}", error),
                        ExpectedOpcodeError::Io(error) => { warn!("IO Error to server, connection broken: {}", error); return; },
                    },
                Ok(opcode) => {
    ...
    fn handle_packet(&self, packet: Box<ServerOpcodeMessage>) {
        match packet.as_ref() {
            ServerOpcodeMessage::SMSG_TIME_SYNC_REQ(req) => {
                ...
                self.send_encrypted( CMSG_TIME_SYNC_RESP { time_sync, client_ticks }).unwrap();
            },
            opcode => self.packet_handler_sender.send(packet).expect("Failed to enqueue packet handlers")
        }
    }
}
